...
- 해당 서비스는 전적으로 구글 애드센스와 연동을 기반으로 하므로, 구글 소셜 로그인이 필수입니다. 또한 개인 애드센스 계정 연동이 필수적으로 요구됩니다.
  - 구글 소셜 로그인을 실제 프로덕션에서 제공하기 위해 검증 절차 진행하고 있어서 24.08.26 기준으로는 이용이 제한될 수 있습니다.
  - 최초 소셜 로그인 후 우측 상단의 [계정ID 조회] 를 1회 진행하면 됩니다.
...
- 현재 사용자가 등록한 알람은 인스턴스가 0 개로 비활성화 될 때 로컬에서 동작하는 node-cron 의 작업 내역이 메모리 상에서 제거 되는 문제가 있습니다.
  - 해당 문제는 GCP 에서 제공하는 스케줄러를 사용하여
