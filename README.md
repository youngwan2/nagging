...
## 🔥 배포
### 8월 27일 반영 예정인 사항 (24.08.26 작성)
- 프로덕션 환경에서 기대하는 동작과 다른 문제가 발견되어서 보완중입니다.
...
- 스케줄 목록과 보고서 등록 이후 추가된 항목이 즉시 렌더링되지 않는 문제가 있고, 로컬에서는 해당 문제를 개선하였습니다.
  - 현재 08.26 기준으로 로컬에서 변경된 부분이 반영되어 있지 않습니다(ex. 웹 사이트 폰트 변경, 로그인 프로필 모달 등등)
  - 현재 배포된 사이트는 GCP의 Cloud 스케줄러 등이 적용 전인 상태이므로, 보고서 설정 후 사용이 제한되는 문제가 있습니다.
  - 모바일 환경에서 레이아웃이 깨지는 문제가 발생하여 로컬에서 해당 문제를 개선하였습니다.
